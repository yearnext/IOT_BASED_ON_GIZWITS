...
|0x01|APP控制状态|只读|1 Byte|NULL|0~1(0为控制成功，1为控制异常)|
|0x02|APP通过该命令设置灯的亮度|读写|灯的亮度|1 Byte|0~255(0为关闭，255为开启)|
|0x03|倒计时设置|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
|0x04|定时开关设置|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|

### (2).插座数据点
|设备命令|设备命令介绍|权限|数据点|数据点大小|参数枚举|
|:-:|:-:|:-:|:-:|:-:|:-:|
|0x00|设备心跳|只读|NULL|NULL|NULL|
|0x01|APP设置数据失败|只读|1 Byte|NULL|0~1(0为控制成功，1为控制异常)|
|0x02|APP通过该命令设置插座的状态|读写|插座的状态|1 Byte|0~1(0为关闭,1为开启)|
|0x03|倒计时设置|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
|0x04|定时开关设置|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
...
|0x00|设备心跳|只读|NULL|NULL|NULL|
|0x01|APP设置数据失败|只读|1 Byte|NULL|0~1(0为控制成功，1为控制异常)|
|0x02|APP通过该命令设置窗帘的状态|读写|窗帘的状态|1 Byte|0~1|
|0x03|倒计时设置|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
|0x04|定时开关设置|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
|0x05|APP通过该命令设置窗帘的工作模式|读写|智能工作模式开关|1 Byte|0~1|
...
|:-:|:-:|:-:|:-:|:-:|:-:|
|0x00|设备心跳|只读|NULL|NULL|NULL|
|0x01|APP读取数据失败|只读|1 Byte|NULL|0~1(0为控制成功，1为控制异常)|
|0x02|APP通过该命令读取室内的温湿度|只读|室内的温度|1 Byte|0~100|
|-|-|-|室内的湿度|1 Byte|0~100|
