...
|0x02|APP通过该命令设置灯的亮度|读写|灯的亮度|1 Byte|0~255(0为关闭，255为开启)|
|0x03|APP通过该命令设置灯的定时器|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
|0x04|APP通过该命令设置灯的定时器|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
...
|0x02|APP通过该命令设置插座的定时器|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
|0x03|APP通过该命令设置插座的定时器|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
...
|0x02|APP通过该命令设置窗帘的定时器|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
|0x03|APP通过该命令设置窗帘的定时器|读写|详见定时器数据类型介绍|详见定时器数据类型介绍|详见定时器数据类型介绍|
...
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|定时器类型|-|-|只读|1 Byte|TIMER_COUNTDOWN_TYPE = 0xA1(倒计时定时器)|
|-|-|-|-|-|TIMER_GENERAL_TYPE = 0xA2(普通定时器)|
|定时器工作模式|-|-|读/写|1 Byte|TIMER_SLEEP_MODE  = 0xB0(定时器休眠模式，定时器停止工作)|
|-|-|-|-|-|TIMER_SIGNAL_MODE = 0xB1(定时器单次运行模式)|
|-|-|-|-|-|TIMER_CUSTOM_MODE = 0xB2(定时器自定义定时模式)|
