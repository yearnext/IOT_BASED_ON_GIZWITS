 ...
 *
 * 按钮收缩    CircularAnimUtil.hide(mChangeBtn);
 * 钮伸展开：  CircularAnimUtil.show(mChangeBtn);
 *
 * 水波般铺满指定颜色并启动一个
 * Activity:CircularAnimUtil.startActivity(MainActivity.this, EmptyActivity.class, view, R.color.colorPrimary);
 * 放图片 CircularAnimUtil.startActivity(MainActivity.this, EmptyActivity.class, view, R.mipmap.img_huoer_black);
 *
 * 在显示或隐藏视图时设置半径和时长
 * 显示：show(View myView, float startRadius, long durationMills)
 * 隐藏：hide(final View myView, float endRadius, long durationMills)
 *
 * startActivity时带上Intent:  startActivity(Activity thisActivity, Intent intent, View triggerView, int colorOrImageRes)
 * startActivityForResult:  startActivityForResult(Activity thisActivity, Intent intent, Integer requestCode, View triggerView, int colorOrImageRes)
 *
 *
 */
