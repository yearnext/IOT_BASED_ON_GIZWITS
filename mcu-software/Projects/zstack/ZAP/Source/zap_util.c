/**************************************************************************************************
    Filename:       zap_util.c
    Revised:        $Date: 2014-12-09 17:49:32 -0800 (Tue, 09 Dec 2014) $
    Revision:       $Revision: 41406 $
 ...
 *
 * @brief       This function parses a packed zclKE_StatusInd_t.
 *
 * input parameters
 *
 * @param       pBuf - A buffer containing a packed zclKE_StatusInd_t.
 *
 * output parameters
 *
 * None.
...
static void zapUtilParseKeyInd(uint8 *pBuf)
{
  ...
  if (NULL != (pInd = (zclKE_StatusInd_t *)osal_msg_allocate(sizeof(zclKE_StatusInd_t))))
  {
    pInd->hdr.event = pBuf[1];
    pInd->hdr.status = pBuf[2];
    pInd->waitTime = pBuf[3];
    pInd->suites = BUILD_UINT16(pBuf[4], pBuf[5]);
